...
from datahub_actions.plugin.action.propagation.propagation_utils import SourceDetails, RelationshipType, DirectionType, \
    PropertyPropagationDirective, PropagationRelationships, PropagationDirective
...
class EntityPropagatorConfig(BaseModel):
    enabled: bool = Field(True, description="Indicates whether entity propagation is enabled.")
...
class EntityPropagator:
    ...
    def __init__(self, action_urn: str, graph: AcrylDataHubGraph, config: EntityPropagatorConfig):
        self.graph = graph
        self.config = config
    ...
    def should_propagate(self, event: EventEnvelope) -> Optional[PropertyPropagationDirective]:
